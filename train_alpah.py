...
def train_net(args):
    ...
    if checkpoint is None:
        model = DIMModel(n_classes=1, in_channels=3, is_unpooling=True, pretrain=True)
...
epoch_result_dir = '/Users/nizhao/xin/causal/code/webzerg/Deep-Image-Matting-PyTorch/our_result_alpha/'
...
def train(train_loader, model, optimizer, epoch, logger):
    ...
    for i, (img, alpha_label) in enumerate(train_loader):
        ...
        loss = alpha_prediction_loss(alpha_out[:, 0, :], alpha_label[:, 0, :])
...
def valid(valid_loader, model, logger, epoch):
    ...
    for img, alpha_label in valid_loader:
        ...
        # save the label input image
        image_name = 'valid_alpha_epoch_' + str(epoch) + '_input_.jpg'
        image_raw = img.detach().cpu().numpy()[0, 0:3, :, :]
        image_data = (image_raw*255).astype(np.uint8)
        Image.fromarray(image_data.transpose(1, 2, 0), 'RGB').save(
            os.path.join(epoch_result_dir, image_name)
        ...
        # save the label alpha image
        image_name = 'valid_alpha_epoch_' + str(epoch) + '_label_.jpg'
        image_raw = gt_alpha.detach().cpu().numpy()[0, 0, :, :]
        image_data = (image_raw*255).astype(np.uint8)
        Image.fromarray(image_data).save(
            os.path.join(epoch_result_dir, image_name)
        ...
        # save the out alpha image
        image_name = 'valid_alpha_epoch_' + str(epoch) + '_out_.jpg'
        image_raw = alpha_out.detach().cpu().numpy()[0, :, :]
        image_data = (image_raw*255).astype(np.uint8)
        Image.fromarray(image_data).save(
            os.path.join(epoch_result_dir, image_name)

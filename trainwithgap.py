...
#     # y = torch.zeros(pred.shape)
#     rows = pred.shape[0]
#     cols = pred.shape[1]
#     for ii in range(rows):
#         for jj in range(cols):
#             ind = torch.argmax(pred[ii, jj])
#             ind = ind.item()
#             if ind == 0:
#                 if (pred[ii, jj, 0] - pred[ii, jj, 1] < 0.1 and pred[ii, jj, 0] - pred[ii, jj, 2] < 0.1):
#                     pred[ii, jj] = torch.tensor([0.0, 1.0, 0.0])
#             elif ind == 2:
#                 if (pred[ii, jj, 2] - pred[ii, jj, 1] < 0.1 and pred[ii, jj, 2] - pred[ii, jj, 0] < 0.1):
#                     pred[ii, jj] = torch.tensor([0.0, 1.0, 0.0])
#


epoch_result_dir = './out_result_gap/'
...
def train(train_loader, model, optimizer, epoch, logger):
    ...
    for i, (img, alpha_label) in enumerate(train_loader):
        ...
        # calculate the loss with confidence level
        trimap_out_t = torch.swapaxes(trimap_out, 1, 2)
        # gt_trimap_flat_t = torch.swapaxes(gt_trimap_flat, 0, 1)
        # new_trimap_out = torch.zeros(trimap_out_t.shape)
        # rows = trimap_out_t.shape[0]
        # cols = trimap_out_t.shape[1]

        rows = trimap_out_t.shape[0]
        cols = trimap_out_t.shape[1]
        for ii in range(rows):
            for jj in range(cols):
                ind = torch.argmax(trimap_out_t[ii, jj])
                ind = ind.item()
                if ind == 0:
                    if (trimap_out_t[ii, jj, 0] - trimap_out_t[ii, jj, 1] < 0.1 and trimap_out_t[ii, jj, 0] - trimap_out_t[ii, jj, 2] < 0.1):
                        trimap_out_t[ii, jj] = torch.tensor([0.0, 1.0, 0.0])
                elif ind == 2:
                    if (trimap_out_t[ii, jj, 2] - trimap_out_t[ii, jj, 1] < 0.1 and trimap_out_t[ii, jj, 2] - trimap_out_t[ii, jj, 0] < 0.1):
                        trimap_out_t[ii, jj] = torch.tensor([0.0, 1.0, 0.0])

        trimap_out_t = torch.swapaxes(trimap_out_t, 1, 2)
        # Calculate loss
        ...
        loss = trimap_loss(trimap_out_t, gt_trimap_flat) #alpha_label is 2 rows (alpha and mask(trimap))
